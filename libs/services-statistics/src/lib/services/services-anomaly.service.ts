...
import { ServicesAnomaly, ServicesEmployeeAnomaly } from '@dbl-dev/services-statistics';
...
export class ServicesAnomalyService {
  ...
  getEmployeeAnomaliesByFileName(fileName: string): Observable<ServicesEmployeeAnomaly[]> {
    return this.http.get<ServicesEmployeeAnomaly[]>(this.apiUrlEvents + '/employee', {
      params: new HttpParams().set('fileName', fileName)
    });
  }
